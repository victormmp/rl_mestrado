...
from rl_mestrado.model.lstm import LSTMActor
...
class DeepActorAgentLearner:
    ...
    def __init__(self, assets: str, n_features: int = 20, n_assets: int = 3, n_layers: int = 20,
    ...
    def train(self, data: pd.DataFrame, epochs: int = 1000, window_size: int = 100, result_output_path: str = '.'):
        ...
        for epoch in range(epochs):
            ...
            hidden = self._actor.init_hidden()
            weights, hidden = self._actor(state, hidden)
            ...
            for date, row in sample_data.iterrows():
                ...
                asset_values = torch.tensor(
                    np.exp(row[[c + '_logReturns' for c in self._assets]].values)
                ).float()
                port_values.append(torch.dot(weights, asset_values))
        ...
        result_df = pd.DataFrame(sharpe,columns=['epoch', self._agent_name]).set_index('epoch')
